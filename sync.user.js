...
// @category       Misc
// @version        0.2.3.20161003.4740
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @updateURL      https://static.iitc.me/build/release/plugins/sync.meta.js
// @downloadURL    https://static.iitc.me/build/release/plugins/sync.user.js
// @description    [iitc-2016-10-03-004740] Sync data between clients via Google Realtime API. Only syncs data from specific plugins (currently: Keys, Bookmarks). Sign in via the 'Sync' link.
...
window.plugin.sync.RegisteredMap.prototype.updateMap = function(keyArray) {
  ...
  try {
    ...
    // console.warn("UpdateMap: Length= "+JSON.stringify(keyArray.length));

    if (_this.fieldName === 'keys'){
      console.warn("UpdateMap: Passed in keys");
      for (i = 0; i < keyArray.length; i++) {
        $.each(Object.keys(keyArray[i]), function(ind, key) {
          var value = window.plugin[_this.pluginName][_this.fieldName][i][key];
          console.warn("UpdateMap: key="+JSON.stringify(key)+ ", value="+value);
      ...
      console.warn("UpdateMap: Passed in else");
...
window.plugin.sync.RegisteredMap.prototype.updateListener = function(e,index) {
  console.warn("updateListener index="+JSON.stringify(index) );
  // debugger;
  if ( index !== undefined){
    if(!e.isLocal) {
      ...
      console.warn("Sync: e.isLocal: False");
    }else{
      console.warn("Sync: e.isLocal: True");
    }
...
window.plugin.sync.RegisteredMap.prototype.loadRealtimeDocument = function(callback) {
  ...
  initializeModel = function(model) {
    ...
    console.warn("loadRealtimeDocument: Length= "+JSON.stringify(window.plugin[_this.pluginName][_this.fieldName].length));
    if (_this.fieldName ==='keys'){
      console.warn("loadRealtimeDocument: Passed in keys");
      var map =[];
      ...
      console.warn("loadRealtimeDocument: Passed in else");
  ...
  onFileLoaded = function(doc) {
    ...
    if (_this.fieldName === 'keys'){
      ...
      _this.map.addEventListener(gapi.drive.realtime.EventType.VALUE_CHANGED, function(event){_this.updateListener(event,undefined);});
    }
